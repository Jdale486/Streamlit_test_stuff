...
number = st.number_input("Enter Team ID", step=1,value=1769272, placeholder = 1769272)
...
FixturesElementsAll3['xGCAvg_Player'] = FixturesElementsAll3['expected_goals_conceded.gw'].apply(lambda x: format(x, ".2f")).astype(str) + '-' + FixturesElementsAll3['web_name']
...
# Sort players by total AttackOpp_pergame and select top 20
top_players = FixturesElementsAll2_totalsAtt.sort_values(by='AttackOpp_pergame', ascending=True).head(NoPlayersShownInt)
...
current_team_data = FixturesElementsAll2AttFiltered.set_index('xGCAvg_Player')['InCurrentTeam'].to_dict()
